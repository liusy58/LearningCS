...
从上面的日志，我们可以看到一共经历了下面几个阶段:

1. 驱动程序运行C编译器(`cc1`)将 `main.c`编译成`main.s`，驱动程序运行汇编器(`as`)将`main.s`翻译成可重定位目标文件
2. 驱动程序运行C编译器(`cc1`)将 `sum.c`编译成`sum.s`，驱动程序运行汇编器(`as`)将`sum.s`翻译成可重定位目标文件
3. 驱动程序运行链接器(`collect2`)将可重定位文件组合起来创建可执行文件`prog`
